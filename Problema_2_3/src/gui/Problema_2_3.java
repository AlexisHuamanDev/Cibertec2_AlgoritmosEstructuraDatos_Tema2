			...
			try {
				...
				try {
					...
					try {
						...
						e.setPrecioDolar(e.getPrecioDolar()*0.2+e.getPrecioDolar());
						listado(e);
					...
					mensaje("Ingrese NUMERO DE PISOS correcto", txtNumPisos);
				}
			}
			catch (Exception e) {
				mensaje("Ingrese NUMERO DE DEPARTAMENTOS correcto", txtNumDptos);
			}
		}
		catch (Exception e) {
			mensaje("Ingrese CODIGO correcto", txtCodigo);
		}	
		
		
	}
	private void listado(Edificio e) {
		
		imprimir("DireMem: " + e);
		imprimir("Codigo piso: " + e.getCodigo());
		imprimir("Numero de departamenos: " + e.getNumDpto());
		imprimir("Cantidad de pisos: " + e.getCantPisos());
		imprimir("Precio del departamento : $/. " +e.getPrecioDolar());
		imprimir("Precio del edificio : $/. " + e.precioEdificio());
		imprimir("");
	}
