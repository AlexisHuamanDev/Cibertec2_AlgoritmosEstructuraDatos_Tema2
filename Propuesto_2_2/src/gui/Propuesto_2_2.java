...
public class Propuesto_2_2 extends JFrame implements ActionListener {
	...
	protected void actionPerformedBtnProcesar(ActionEvent arg0) {
		...
		Paciente p = new Paciente ("Pedro","Quispe",20,1.50,62.5);
		listado(p);
		
		p.setEdad((15));
		listado(p);
	}
	private void listado(Paciente p) {
		imprimir("Direccion de memoria: "+p);
		imprimir("Nombre: "+p.getNombre());
		imprimir("Apellido: "+p.getApellido());
		imprimir("Edad : "+p.getEdad());
		imprimir("Talla :"+p.getTalla());
		imprimir("Peso: "+p.getPeso());
		imprimir("	" + p.determinarEdad());
		imprimir("");
